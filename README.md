...
## Цель и задачи
Целью исследования является оценка возможности использования глубокого обучения для сегментации сельскохозяйственных полей на примере Ставропольского края. Для достижения данной цели был поставлен ряд задач необходимых для решения:
...
2. Обучение модели
    ...
    - Непосредственной обучение нейронной сети
...
Нейронная сеть обучалась на ресурсах kaggle, так как у автора отсутствует необходимые мощности GPU (в дальнейшем сеть работала локально на gpu GeForce 1050 mobile 4gb).Ноутбук  процессом обучения - [sentinel-segmentation_UNET.ipynb](sentinel-segmentation_UNET.ipynb)
...
Выбор U-net обусловлен наиболее частой встречаемость данной архитектуры для работы с данными ДЗЗ
...
Итоговой частью работы был сбор воедино скриптов посвещенных получению обработке, работе обученного UNet и итоговой сборке векторного изображения с/х полей интересующего региона. 
...
1. На старте у пользователя есть только геометрия отражающая интересующий его регион(далее по тексту стартовая геометрия), в нашем случае это просто радномный    [прямоугольник](data/geoms/example_geometry_WGS84.geojson) (в моем случае этот прямоугольник уже в метрической проекции ЮТМ 38 зона N)
...
4. Далее полученный в шаге 2 растр режется ан патчи а метаданные о патчах сохраняются в [patches_meta.json](/data/temp/patches_meta.json)
...
3. Над чем следует работать 
    - Имеет обернуть имеющийся код в docker для простоты его использования другими пользователями
