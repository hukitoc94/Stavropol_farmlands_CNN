...
В ноутбуке [project.ipynb](project.ipynb) подробно описан процесс работы всех скриптов и инструментов, здесь я ограничусь тезисным описанием процесса работы:

1. На старте у пользователя есть только геометрия отражающая интересующий его регион, в нашем случае это просто радномный    [прямоугольник](data/geoms/example_geometry.geojson) (в моем случае этот прямоугольник уже в метрической проекции ЮТМ 38 зона N)
